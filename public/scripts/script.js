window.addEventListener('load', function (e) {
    ...
    // const totalcount = docment.querySelector('.allcount');
    ...
    countPrice.forEach((product, index) => {
        product.innerHTML = toCurrency(price[index].innerHTML * count[index].innerHTML)
                    ...
                    if (card.products.length > 0) {
                        const html = card.products.map(product => {
                            ...
                            <tr>
                                    ...
                                    <td class="count_price">${toCurrency(+product.price * product.count)}</td>
                                    <td>
                                        <button id="${product.id}" data-id="${product.id}" class="btn remove-btn" style="background-color: red;">Delete</button>
                                    </td>
                                </tr>
                            `
                        }).join('')
                        ...
                        cardBox.querySelector('.allcount', '.summ').innerHTML = card.count
